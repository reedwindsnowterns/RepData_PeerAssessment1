...
The interval with the highest average daily step count is `r maxIntvl` with `r maxIntvlSteps` steps. To address the missing values, 
...
After the imputation of missing values, there are `r nullStepCount` missing values in the dataset and the new mean and median are `r actDailyMean_imp` and `r actDailyMed_imp` respectively. The operation removed a number of steps previously skewing the data and caused the two measures of central tendency to converge. 
